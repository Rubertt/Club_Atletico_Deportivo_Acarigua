        ...
        <div class="data-table-wrap" style="border: none; border-radius: 0; border-top: 1px solid var(--color-border); max-height: 500px; overflow-y: auto;">
            ...
            <div class="asig-headers-desktop" style="display: flex; align-items: center; gap: 16px; padding: 12px 24px; background: var(--color-bg-alt); border-bottom: 1px solid var(--color-border); position: sticky; top: 0; z-index: 10; font-size: 13px; font-weight: 600; color: var(--color-text-muted);">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr 120px; gap: 16px; flex: 1;">
                    <div data-tooltip="Posición principal en la cancha en la que se desempeña el atleta." data-tooltip-pos="top" style="cursor: help;">Posición Principal</div>
                    <div data-tooltip="Posición táctica alternativa. Debe ser distinta a la posición principal seleccionada." data-tooltip-pos="top" style="cursor: help;">Posición Secundaria</div>
                    <div data-tooltip="Número de camiseta del atleta. Debe ser un número del 1 al 999 y no puede repetirse dentro de esta categoría." data-tooltip-pos="top" style="cursor: help;">Dorsal</div>
                </div>
            </div>

            <!-- Listado de atletas candidatos -->
            ...
            ?>
                <div class="asig-atleta-row" style="<?= $isDisabled ? 'opacity: 0.65; background: var(--color-bg-alt);' : '' ?>">
                    <div class="asig-atleta-row__athlete">
                        ...
                        <?php else: ?>
                            <div class="avatar-placeholder" style="width: 38px; height: 38px; border-radius: 50%; background: var(--color-primary-light); color: var(--color-primary); display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 12px; border: 1px solid var(--color-primary-light); flex-shrink: 0;">
                        ...
                        <div class="asig-atleta-row__name-wrap">
                            ...
                            <div class="asig-atleta-row__meta">
                                <?= e(date('d/m/Y', strtotime($a['fecha_nac']))) ?> (<?= (int)$a['edad'] ?> años)
                            </div>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateRowStates() {
        ...
        checkboxes.forEach(cb => {
            const row = cb.closest('.asig-atleta-row');
    ...
    document.querySelectorAll('.select-posicion-principal').forEach(select => {
        select.addEventListener('change', (e) => {
            const row = e.target.closest('.asig-atleta-row');
            updateSecondaryOptions(row);
        });
    });
